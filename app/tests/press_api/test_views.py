...
class CategoryRankingTest(APITestCase):
    # API가 요청을 올바르게 처리하고 응답을 반환하는지 확인하기 위한 테스트
    def test_get_queryset(self):
        url = reverse("categoryranking")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    # API 엔드포인트에 GET 요청을 보내고 "sort=category_name" 매개변수를 포함하여
    # 응답 코드가 HTTP 200 OK인지 확인합니다. 이는 API가 정렬된 결과를 반환하는지 확인하기 위한 테스트
    def test_get_queryset_with_sort(self):
        url = reverse("categoryranking") + "?sort=category_name"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    # API 엔드포인트에 GET 요청을 보내고 "number=2" 매개변수를 포함하여 응답 코드가 HTTP 200 OK인지 확인
    # 이는 API가 특정 개수의 결과만 반환하는지 확인하기 위한 테스트
...
class CategoryDetailTest(APITestCase):
    # 카테고리를 생성하고 해당 카테고리에 대한 상세 정보를 가져오는 테스트
    def test_get_category_detail(self):
        category_name = "종합지"
        Category.objects.create(category_name=category_name)
        # reverse()를 사용하여 URL 패턴의 이름을 기반으로 실제 URL을 생성
